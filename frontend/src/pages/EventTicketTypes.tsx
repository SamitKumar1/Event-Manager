    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-text mt-1">{event.title} - Ticket Types</h1>
        </div>
        <button
      ...
      <div className="bg-surface rounded-lg shadow-sm border border-border overflow-hidden">
        {ticketTypes.length === 0 ? (
          <div className="p-12 text-center">
            <p className="text-text-muted mb-4">No ticket types yet</p>
            <p className="text-sm text-text-subtle mb-4">Create ticket types to allow attendees to purchase tickets</p>
            <button
              onClick={() => setShowCreateModal(true)}
              className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-primary-600 hover:bg-primary-700"
            >
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <div className="mt-1 flex items-center space-x-4 text-sm text-text-muted">
              ...
              <span>Qty: <span className="font-medium text-text">{ticketType.quantity}</span></span>
              <span>
                Sales: <span className="font-medium text-text">{formatDate(ticketType.salesStartAt)} - {formatDate(ticketType.salesEndAt)}</span>
              </span>
              <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${isOnSale ? 'bg-success-soft text-success' : 'bg-surface-2 text-text'}`}>
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            className="px-3 py-1.5 text-sm font-medium text-danger hover:text-danger"
          >
            Delete
          </button>
        </div>
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex min-h-full items-center justify-center p-4">
        ...
        <div className="relative bg-surface rounded-lg shadow-xl max-w-md w-full p-6">
          <h2 className="text-xl font-bold text-text mb-4">Create Ticket Type</h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <label className="block text-sm font-medium text-text-muted mb-1">Price (USD)</label>
                <input
                  type="number"
                  step="0.01"
                  min="0"
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <label className="block text-sm font-medium text-text-muted mb-1">Sales Start</label>
                <input
                  type="datetime-local"
              ...
              <div>
                <label className="block text-sm font-medium text-text-muted mb-1">Sales End</label>
                <input
                  type="datetime-local"
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex min-h-full items-center justify-center p-4">
        <div className="fixed inset-0 bg-black/50 transition-opacity" onClick={onClose} />
        <div className="relative bg-surface rounded-lg shadow-xl max-w-md w-full p-6">
          <h2 className="text-xl font-bold text-text mb-4">Edit Ticket Type</h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            {error && <div className="bg-danger-soft text-danger p-3 rounded-md text-sm">{error}</div>}
            <div>
              <label className="block text-sm font-medium text-text-muted mb-1">Name</label>
